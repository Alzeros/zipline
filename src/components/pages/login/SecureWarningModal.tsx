    ...
    <Modal opened={opened} onClose={onClose} title={t('HTTPS Configuration')} size='lg'>
      ...
      <Text mt='md'>
        {returnHttps ? (
          <>
            {t(
              'To resolve this issue, please access this instance through HTTPS. If that is currently not possible, you can temporarily set the',
            )}{' '}
            <Code>CORE_RETURN_HTTPS_URLS</Code> {t('environment variable to')} <Code>false</Code>。
          </>
        ) : (
          <>
            {t(
              'To resolve this issue, it is recommended to have your server configured to use HTTPS. This can be done by setting the',
            )}{' '}
            <Code>CORE_RETURN_HTTPS_URLS</Code> {t('environment variable to')} <Code>true</Code>
            {t(
